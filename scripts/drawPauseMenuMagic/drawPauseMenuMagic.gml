...
for (var i=0; i<ds_list_size(player.magic); i++){
	var new_category=World.all_magic[player.magic[| i]].school;
...
if (pause_greater==PauseMagicScreens.base){
	...
	if (is_regular_magic(list_category[| pause_greater_position])){
		switch (World.all_magic[list_magic[| pause_lesser_position]].school){
	...
	for (var i=0; i<ds_list_size(list_magic); i++){
		...
		switch (list_category[| pause_greater_position]){
			...
			case MagicClasses.powers:
				name=World.all_powers[list_magic[| i].index].name;
				break;
			case MagicClasses.shout:
				name=World.all_shouts[list_magic[| i].index].name;
				break;
			default:
				...
				var name=World.all_magic[list_magic[| i]].name;
				break;
		}
		draw_set_halign(fa_left);
	...
	switch (pause_greater){
		case PauseMagicScreens.contents:
			...
			switch (list_category[| pause_greater_position]){
				case MagicClasses.effects:
					...
					break;
				case MagicClasses.powers:
					draw_text(center_x, center_y, World.all_powers[thing.index].name);
					draw_set_font(FontPauseRegular);
					draw_text_ext(center_x, center_y_b, World.all_powers[thing.index].description, -1, text_width);
					break;
				case MagicClasses.shout:
					var base_shout=World.all_shouts[thing.index];
					...
					break;
				default:
					draw_text(center_x, center_y, World.all_magic[thing].name);
					draw_set_font(FontPauseRegular);
					draw_text_ext(center_x, center_y_b, World.all_magic[thing].description, -1, text_width);
					break;
			}
			if (Controller.release_up){
		...
		case PauseMagicScreens.unlock:
			...
			draw_set_valign(fa_middle);
			draw_set_color(c_white);
			draw_text_ext(center_x, center_y, "Would you like to spend one Dragon Soul to unlock the next word of "+World.all_shouts[thing.index].name+"?", -1, text_width);
		...
		case PauseMagicScreens.cantunlock:
			...
			draw_set_valign(fa_middle);
			draw_set_color(c_white);
			draw_text_ext(center_x, center_y, "You do not have any Dragon Souls to unlock the next word of "+World.all_shouts[thing.index].name+" with.", -1, text_width);
		...
		case PauseMagicScreens.cantuse:
			...
			draw_set_valign(fa_middle);
			draw_set_color(c_white);
			draw_text_ext(center_x, center_y, "There are no words available to unlock for "+World.all_shouts[thing.index].name+".", -1, text_width);
			draw_text(center_x, y_response, "Okay");
			draw_sprite(spr_menu_arrow, 0, center_x-64, y_response);
			if (Controller.release_a||Controller.release_b){
				pause_greater=PauseMagicScreens.contents;
