...
draw_text(x_sp, top_y+pause_menu_offset, World.all_races[player.race].name);
...
for (var i=-2; i<=2; i++){
	...
	draw_sprite(World.all_skills[n].sprite, 0, position*W/10, 720/3);
}

draw_text_ext(center_x, text_description_y, World.all_skills[pause_greater_position].description, -1, text_width);
...
switch (pause_greater){
	...
	case PauseSkillScreens.perk:
		var skill=World.all_skills[pause_greater_position];
