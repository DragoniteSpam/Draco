function process_player_controls_post() {
	...
	if (free){
		...
		// Attack thing
		if (Controller.release_x&&can_attack){
			Controller.release_x=false;
			OVERWORLD_ATTACK(CombatStyles.physical, Controller.time_x);
		}
		// Magicka thing
		if (Controller.release_y){
			Controller.release_y=false;
			OVERWORLD_ATTACK(CombatStyles.magical, Controller.time_y);
		}
		// Pause
		...
		// Auto activate
		if (instanceof_classic(World.event[# xx, yy], Event)){
