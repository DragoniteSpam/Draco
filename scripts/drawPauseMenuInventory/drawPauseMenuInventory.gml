...
if (argument_count==1&&argument[0]!=noone){
	...
	for (var i=0; i<ds_list_size(char.inventory); i++){
		...
		var new_category=World.all_items[char.inventory[| i].index].class;
...
for (var i=0; i<ds_list_size(player.inventory); i++){
	var new_category=World.all_items[player.inventory[| i].index].class;
	if (category!=new_category){
		if (new_category==ItemClasses.misc){
...
if (pause_greater==PauseInventoryScreens.base){
	...
	switch (pause_greater){
				...
				if (owner==player){
					...
					if (pause_exchange_money){
						if (stack.quantity>5){
							...
							if (GetGold(Actors.playable)>=amt){
								...
								RemoveItem(stack.index, 1, argument[0]);
							}
						}
					// If you may take freely
						...
						if (stack.quantity>5&&stack.index!=Items.gold){
							...
							RemoveItem(stack.index, n, argument[0]);
						}
					}
				}
			} else if (Controller.release_b||Controller.release_left){
				...
				if (owner==player){
					...
					if (pause_exchange_money){
						...
						if (GetGold(argument[0])>=cost){
							...
							RemoveItem(stack.index, qty, Actors.playable);
						} else {
							...
							RemoveItem(stack.index, safe_qty, Actors.playable);
						}
					...
					if (pause_exchange_money){
						...
						if (GetGold(Actors.playable)>=cost){
							...
							RemoveItem(stack.index, qty, argument[0]);
						} else {
							...
							RemoveItem(stack.index, safe_qty, argument[0]);
						}
					// If you may take freely
					} else {
						if (owner!=noone){
		...
		case PauseInventoryScreens.poison:
			...
			var base_weapon=World.all_items[stack_weapon.index];
